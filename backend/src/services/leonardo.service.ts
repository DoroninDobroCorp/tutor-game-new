...
export async function startImageGeneration(params: GenerateImageParams): Promise<{ generationId: string }> {
    ...
    try {
        ...
        if (!generationId) {
            ...
            throw new AppError(`Failed to start Leonardo generation job. API Response: ${JSON.stringify(generationResponse.data)}`, 500);
        }
        ...
        throw new AppError(`Failed to communicate with Leonardo AI: ${error.response?.data?.error || error.message}`, 500);
    }
}

export async function uploadImageToLeonardo(filePath: string): Promise<{ imageId: string }> {
    if (!config.leonardoApiKey) {
        throw new AppError('Leonardo API key is not configured.', 500);
    }

    // Step 1: Get upload URL
    const initResponse = await axios.post(`${LEONARDO_API_URL}/init-image`, {
        extension: path.extname(filePath).substring(1) // "jpg", "png" etc.
    }, { headers: HEADERS });

    const uploadData = initResponse.data?.uploadInitImage;
    if (!uploadData || !uploadData.id) {
        throw new AppError('Failed to initialize image upload. API Response: ' + JSON.stringify(initResponse.data), 500);
    }
