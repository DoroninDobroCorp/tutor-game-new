...
export const createAndSendMessage = async (
    ...
    try {
        ...
        console.log(`[Chat Service] Sent system message from ${senderId} to ${recipientId}`);

        return newMessage;
    } catch (error) {
        console.error(`[Chat Service] Failed to send system message from ${senderId} to ${recipientId}`, error);
