...
} from "../services/leonardo.service";
...
export const generateStorySnippetHandler = async (
  req: Request,
  res: Response,
) => {
  ...
  const { setting, studentAge, language, characterPrompt } =
    lesson.section.learningGoal;

  const allGoalSections = await prisma.contentSection.findMany({
    where: { learningGoalId: lesson.section.learningGoal.id },
    orderBy: { order: "asc" },
    include: {
      lessons: { orderBy: { order: "asc" }, include: { storyChapter: true } },
    },
  });
  ...
  for (const prevLesson of previousLessons) {
    if (prevLesson.storyChapter) {
      const { teacherSnippetText, studentSnippetText } =
        prevLesson.storyChapter;
      let chapterContext = "";
  ...
  const storyContext: string | undefined =
  ...
  try {
    ...
    const { storyText, imagePrompt, useCharacterReference } =
      await generateStorySnippet(
        lesson.title,
        setting,
        studentAge,
    ...
    console.log(
      "[PROMPT SUGGESTION] Generated for story snippet:",
      imagePrompt,
    );
    console.log(
      "[PROMPT SUGGESTION] Use character reference:",
...
export const checkStoryImageStatusHandler = async (
  req: Request,
  res: Response,
) => {
...
export const approveStorySnippetHandler = async (
  req: Request,
  res: Response,
) => {
  const { lessonId } = req.params;
  ...
  wsService.emitToUser(
    lesson.section.learningGoal.studentId,
    "teacher_reviewed_lesson",
    {
...
export const approveStorySnippetWithUploadHandler = async (
  req: Request,
  res: Response,
) => {
  const { lessonId } = req.params;
...
export const regenerateStoryImageHandler = async (
  req: Request,
  res: Response,
) => {
  const { lessonId } = req.params;
  ...
  if (useCharacterReference) {
    ...
    if (!characterImageUrl && !characterImageId) {
      console.warn(
        `[STORY REGEN] AI requested character reference, but no character is set for goal ${lesson.section.learningGoal.id}. Proceeding without reference.`,
      );
    } else {
      ...
      if (characterImageUrl && characterImageUrl.startsWith("/uploads/")) {
        ...
        if (fs.existsSync(imagePath)) {
          try {
            console.log(
              `[LEONARDO.AI] Regenerating: Uploading character reference from: ${imagePath}`,
            );
            const { imageId } = await uploadImageToLeonardo(imagePath);
            referenceImageId = imageId;
            console.log(
              `[LEONARDO.AI] Regenerating: Got fresh reference imageId: ${referenceImageId}`,
            );
          } catch (uploadError) {
            console.error(
              "Failed to upload character image for regeneration, proceeding without it.",
              uploadError,
            );
