...
export const createGoalHandler = async (req: Request, res: Response) => {
  ...
  if (!studentId || !subject || !setting || studentAge === undefined) {
    throw new AppError(
      "All fields (studentId, subject, setting, studentAge) are required",
      400,
    );
  }

  const age =
    typeof studentAge === "string" ? parseInt(studentAge, 10) : studentAge;
...
export const getGoalStoryHistoryHandler = async (
  req: Request,
  res: Response,
) => {
  const { goalId } = req.params;
