...
export const generateCharacterHandler = async (req: Request, res: Response) => {
  ...
  while (attempts < maxAttempts) {
    ...
    if (imageResult.status === "COMPLETE" && imageResult.url) {
      ...
      throw new AppError(
        "Failed to generate character image from Leonardo.",
        500,
      );
    }

    console.log(
      `[LEONARDO.AI] Image generation in progress (attempt ${attempts}/${maxAttempts})`,
    );
  }
...
export const uploadCharacterImageHandler = async (
  req: Request,
  res: Response,
) => {
  const { goalId } = req.params;
...
export const updateCharacterPromptHandler = async (
  req: Request,
  res: Response,
) => {
  const { goalId } = req.params;
