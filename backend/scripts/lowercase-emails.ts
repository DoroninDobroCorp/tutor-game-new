...
async function lowercaseEmails() {
  ...
  if (duplicates.length > 0) {
    console.error(
      "❌ ABORTING: Found emails that would become duplicates when lowercased.",
    );
    console.error(
      "Please resolve these manually before running the script again:",
    );
    for (const [email, ids] of duplicates) {
      console.error(
        `- Email "${email}" is used by user IDs: ${ids.join(", ")}`,
      );
      ...
      conflictingUsers.forEach((u) =>
        console.log(`  - User ID: ${u.id}, Original Email: ${u.email}`),
      );
    }
    process.exit(1);
    return;
  ...
  console.log(
    "✅ No potential duplicate emails found. Proceeding with update.",
  );
  ...
  const transaction = users
    ...
    .map((user) => {
      console.log(
        `- Updating email for user ${user.id}: ${user.email} -> ${user.email.toLowerCase()}`,
      );
