  ...
  if (!videoId)
    return (
      <div className="text-red-500 p-4 bg-red-50 rounded-md my-4">
        {t("studentAdventure.invalidYouTubeLink")}
      </div>
    );
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                <Dialog.Title
                  as="h3"
                  className="text-lg font-medium leading-6 text-gray-900 flex justify-between items-start"
                >
...
const ConfettiPiece = ({
  ...
  <div className={`absolute w-2 h-4 rounded-full ${color}`} style={style} />
);
  ...
  return (
    <div className="absolute inset-0 pointer-events-none z-0">
      <style>{`
            ...
            `}</style>
      {pieces.map((p) => (
        <ConfettiPiece key={p.id} style={p.style} color={p.color} />
      ))}
    </div>
  );
};
...
export default function StudentAdventurePage() {
  ...
  const [practiceChat, { isLoading: isChatLoading }] =
    useLessonPracticeChatMutation();
  ...
  useEffect(() => {
    if (lesson) {
      ...
      if (lesson.type === "CONTROL_WORK") {
        ...
        setChatHistory(
          cwQuestions.length > 0
            ? [{ role: "assistant", content: cwQuestions[0].content }]
            : [],
        );
      } else {
  ...
  const handleStoryResponseChange = (
  ...
  const handleNextBlock = () => {
    if (
      ...
      !practiceAnswers[currentBlockIndex]?.trim()
    ) {
  ...
  const startAssessmentPhase = async () => {
    ...
    try {
      ...
      setChatHistory([
        { role: "assistant", content: result.data.responseText },
      ]);
    } catch (err) {
  ...
  const handleStudentChatSubmit = async (e: React.FormEvent) => {
    ...
    const newHistory: ChatMessage[] = [
      ...chatHistory,
      { role: "user", content: studentChatMessage },
    ];
  ...
  const handleControlWorkSubmit = async (e: React.FormEvent) => {
    ...
    const currentFullHistory: ChatMessage[] = [
      ...chatHistory,
    ...
    try {
      ...
      if (isSessionComplete) {
        ...
        const newProgress =
          requiredAnswers > 0 ? (newCorrectCount / requiredAnswers) * 100 : 0;
        if (newProgress >= 100) {
          toast.success(t("studentAdventure.submitControlWorkSuccess"), {
            duration: 3000,
          });
        ...
        if (nextIndex < controlWorkQuestions.length) {
          ...
          setChatHistory((prev) => [
            ...
            {
              role: "assistant",
              content: controlWorkQuestions[nextIndex].content,
            },
          ]);
        } else {
          const commandHistory: ChatMessage[] = [
            ...currentFullHistory,
            { role: "user", content: "GENERATE_NEW_QUESTION_ON_FAILED_TOPIC" },
          ];
          const newQuestionResult = await practiceChat({
            lessonId: lesson.id,
            chatHistory: commandHistory,
          }).unwrap();
          const { responseText: newQResponseText, newQuestion: newQ } =
  ...
  const handleSubmitLesson = async () => {
    ...
    const progress =
      requiredAnswers > 0 ? (correctAnswers / requiredAnswers) * 100 : 0;
    const isControlWorkSuccess =
      lesson.type === "CONTROL_WORK" && progress >= 100;
    ...
    try {
      ...
      toast.success(t("studentAdventure.lessonSubmitSuccess"), {
        duration: 4000,
      });
      dispatch(resetAdventureState());
      navigate("/student");
    ...
    return (
      <div className="text-center text-red-500 p-10">
        <Trans i18nKey="studentAdventure.lessonLoadingError">
          Ошибка загрузки урока. Пожалуйста,{" "}
  ...
  const renderContent = () => {
      ...
      return (
        <div className="text-center p-10 bg-white rounded-lg shadow">
          <h2 className="text-2xl font-bold text-green-600">
            {t("studentAdventure.congratulations")}
          </h2>
          <p className="mt-4 text-lg text-gray-700">
            {t("studentAdventure.allLessonsCompleted")}
          </p>
          <button
            onClick={() => navigate("/student")}
            className="mt-6 btn-primary"
          >
            {t("studentAdventure.backToCabinet")}
          </button>
        </div>
      );
    }
        ...
        <div className="bg-gray-50 rounded-lg shadow-md p-6">
          <div className="flex justify-between items-start mb-4">
            <h2 className="text-2xl font-bold text-gray-700">
              {t("studentAdventure.storyContinuation")}
            </h2>
            <button
              onClick={() => {}}
              className="px-3 py-2 text-sm bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 flex items-center gap-2"
            >
          ...
          {lesson.storyChapter ? (
            <>
              <div className="flex flex-col md:flex-row gap-6 items-start">
                ...
                <p className="flex-1 text-gray-700 leading-relaxed italic whitespace-pre-wrap">
                  {lesson.storyChapter.teacherSnippetText}
                </p>
              </div>
              <div className="mt-6">
                <label
                  htmlFor="storyResponse"
                  className="block text-lg font-semibold text-gray-800 mb-2"
                >
                  {t("studentAdventure.whatWillYouDoNext")}
                </label>
                <textarea
                  id="storyResponse"
                  rows={4}
                  ...
                  value={storyResponse}
                  onChange={handleStoryResponseChange}
                  placeholder={t("studentAdventure.writeYourAction")}
    ...
    if (lesson.type === "CONTROL_WORK" && isControlWorkComplete) {
      return (
        <div className="text-center p-10 bg-white rounded-lg shadow-lg relative overflow-hidden">
          <Confetti />
          <h2 className="text-3xl font-bold text-green-600 z-10 relative">
            {t("studentAdventure.congratulations")}
          </h2>
          <p className="mt-4 text-lg text-gray-700 z-10 relative">
            {t("studentAdventure.controlWorkPassed")}
          </p>
          <button
    ...
    if (lessonPhase === "control_work") {
      const progress =
        requiredAnswers > 0
          ? Math.min(100, (correctAnswers / requiredAnswers) * 100)
          : 0;
      return (
        <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
          ...
          <div>
            ...
            <p className="text-center text-sm text-gray-600 mt-2">
              {t("studentAdventure.progress")}: {Math.round(progress)}% (
              {correctAnswers} / {requiredAnswers})
            </p>
          </div>
          <div
            ref={chatContainerRef}
            className="h-96 bg-gray-50 rounded-lg p-3 space-y-4 overflow-y-auto"
          >
            {chatHistory.map((msg, index) => (
              ...
              >
                <div
                  className={`max-w-lg px-4 py-2 rounded-lg shadow ${msg.role === "user" ? "bg-blue-500 text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none border"}`}
          ...
          <form onSubmit={handleControlWorkSubmit} className="flex gap-2">
            <input
              ...
              placeholder={t("studentAdventure.yourAnswerPlaceholder")}
              className="input flex-1"
              disabled={isChatLoading}
            />
            <button
              type="submit"
              disabled={!studentChatMessage.trim() || isChatLoading}
          ...
          <div className="text-center pt-4 border-t">
            <button
              ...
              className="text-sm text-gray-500 hover:text-red-600 flex items-center justify-center mx-auto gap-2"
            >
              {" "}
              <FiThumbsDown /> {t("studentAdventure.giveUp")}
            </button>
          </div>
        </div>
      );
      ...
      <>
        ...
        {lessonPhase === "content" && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div
              className={`p-4 rounded-r-lg ${currentBlock.type === "theory" ? "block-theory" : currentBlock.type === "practice" ? "block-practice" : "block-alert"}`}
            >
              <h3 className="font-semibold capitalize text-lg mb-2">
                {currentBlock.type === "youtube"
                  ? t("studentAdventure.video")
                  : currentBlock.type}
              </h3>
              {currentBlock.type === "youtube" ? (
              ...
              {currentBlock.type === "practice" && (
                <div className="mt-4">
                  <label
                    htmlFor={`answer-${currentBlockIndex}`}
                    className="block text-sm font-medium text-gray-700 mb-1"
                  >
                    {t("studentAdventure.yourAnswerLabel")}:
                  </label>
                  <textarea
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              >
                {currentBlockIndex >= blocks.length - 1
                  ? t("studentAdventure.finishLesson")
                  : t("studentAdventure.next")}{" "}
                <FiChevronRight />
              </button>
            </div>
          </div>
        ...
        {lessonPhase === "assessment" && (
          <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
            <div className="flex items-center gap-3 text-xl font-semibold text-gray-800">
              <FiZap />
              {t("studentAdventure.assessmentTitle")}
            </div>
            <div
              ref={chatContainerRef}
              className="h-96 bg-gray-50 rounded-lg p-3 space-y-4 overflow-y-auto"
            >
              {chatHistory.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-lg px-4 py-2 rounded-lg shadow ${msg.role === "user" ? "bg-gray-500 text-white rounded-br-none" : "bg-white text-gray-800 rounded-bl-none"}`}
                  >
                    {msg.content}
                  </div>
                </div>
            ...
            <div className="text-center pt-4 border-t">
              <button
                onClick={() => {
                  if (lesson) toast.error(t("common.notImplemented"));
                }}
                disabled={isEndingLesson}
                className="text-sm text-gray-500 hover:text-red-600 flex items-center justify-center mx-auto gap-2 disabled:opacity-50"
              >
                {" "}
                <FiCoffee /> {t("studentAdventure.giveUp")}
              </button>
            </div>
          </div>
        )}
