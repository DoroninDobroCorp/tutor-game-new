...
const CompletedLessonsPage = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetCompletedLessonsQuery(goalId!, {
    skip: !goalId,
  });
  ...
  if (isError) {
    return (
      <div className="text-center p-8 text-red-500">
        {t("completedLessons.loadingError")}
      </div>
    );
  }
                  ...
                  >
                    <button
                      onClick={() =>
                        setExpandedLessonId((prev) =>
                          prev === compLesson.id ? null : compLesson.id,
                        )
                      }
                    ...
                    {expandedLessonId === compLesson.id && (
                      <div className="px-3 pb-3 border-t border-gray-200">
                        {(compLesson.content?.blocks || []).map(
                          (block, index) => {
                            const log = compLesson.performanceLogs?.find(
                              (l) => l.blockIndex === index,
                            );
                            return (
                              <div
                                key={index}
                              ...
                              >
                                <div
                                  className={`p-3 border-l-4 rounded-r-md ${block.type === "theory" ? "border-blue-400 bg-blue-50" : "border-purple-400 bg-purple-50"}`}
                                >
                                  <h4 className="font-semibold capitalize text-gray-700">
                                    {block.type === "theory"
                                      ? t("completedLessons.theory")
                                      : t("completedLessons.practice")}
                                  </h4>
                                  <p
