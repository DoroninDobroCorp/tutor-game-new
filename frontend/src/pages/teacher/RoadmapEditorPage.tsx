...
const RoadmapEditorPage = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useGetLearningGoalByIdQuery(goalId!, {
    skip: !goalId,
  ...
  const [getLogs, { data: performanceLogs, isLoading: isLoadingLogs }] =
    useLazyGetPerformanceLogsQuery();
  const [getStoryHistory, { data: storyHistory, isLoading: isLoadingStory }] =
    useLazyGetGoalStoryHistoryQuery();
  ...
  const handleGeneratePlan = async (e: React.FormEvent) => {
    e.preventDefault();
    const messageToSend =
      userMessage.trim() || t("roadmapEditor.generateInitialPlan");
    const newHistory = [
      ...chatHistory,
    ...
    try {
      ...
      const newSections = result.roadmap.map(
        (s: any, sectionIndex: number) => ({
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (type === "SECTIONS") {
      ...
      const sourceSectionIndex = parseInt(
        source.droppableId.replace("lessons-", ""),
        10,
      );
      const destSectionIndex = parseInt(
        destination.droppableId.replace("lessons-", ""),
        10,
      );
      const [movedLesson] = newRoadmap[sourceSectionIndex].lessons.splice(
        source.index,
        1,
      );
      newRoadmap[destSectionIndex].lessons.splice(
        destination.index,
        0,
        movedLesson,
      );
  ...
  const handleAddSection = () =>
    setRoadmap([
      ...roadmap,
      {
        ...
        title: t("roadmapEditor.newSection", { number: roadmap.length + 1 }),
  ...
  const handleAddLesson = (sectionIndex: number) => {
    ...
    section.lessons.push({
      ...
      title: t("roadmapEditor.newLesson"),
      type: "THEORY",
      status: "DRAFT",
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        >
          <FiArrowLeft className="mr-2" /> {t("roadmapEditor.back")}
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4">
              {t("roadmapEditor.storyCharacter")}
            </h2>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="flex gap-2">
                <button
                  onClick={async () => {
                    if (!goalId) return;
                    await getLogs({
                      studentId: currentGoal.student.id,
                      goalId,
                    }).unwrap();
                    setShowLogsModal(true);
                  }}
                  className="text-sm flex items-center gap-2 px-3 py-1.5 bg-yellow-100 text-yellow-800 rounded-md hover:bg-yellow-200"
                >
                  <FiEye /> {t("roadmapEditor.viewAnswers")}
                </button>
                <button
                  onClick={async () => {
                    if (!goalId) return;
                    getStoryHistory(goalId);
                    setShowStoryModal(true);
                  }}
                  className="text-sm flex items-center gap-2 px-3 py-1.5 bg-blue-100 text-blue-800 rounded-md hover:bg-blue-200"
                >
                  <FiBookOpen /> {t("roadmapEditor.viewStory")}
                </button>
              </div>
            </div>
                  ...
                  >
                    {roadmap.map((section, sectionIndex) => (
                      <RoadmapSection
                        ...
                        startEditing={(si, li) =>
                          setIsEditingTitle({ section: si, lesson: li })
                        }
                        stopEditing={() =>
                          setIsEditingTitle({ section: null, lesson: null })
                        }
                      />
                    ))}
            ...
            <div className="mt-8">
              ...
              >
                <FiPlus className="mr-2" /> {t("roadmapEditor.addSection")}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1 bg-white rounded-lg shadow-md p-6 flex flex-col h-[calc(100vh-12rem)]">
          <h2 className="text-xl font-semibold mb-4 text-center">
            {t("roadmapEditor.aiDialog")}
          </h2>
          <div
            ref={chatContainerRef}
            className="flex-grow bg-gray-50 rounded-lg p-3 overflow-y-auto space-y-3 mb-4"
          >
            {chatHistory.length === 0 && (
              <div className="text-center text-gray-400 h-full flex items-center justify-center">
                {t("roadmapEditor.startDialog")}
              </div>
            )}
            {chatHistory.map((msg, idx) => (
              <div
                key={idx}
                className={`p-3 rounded-lg max-w-[90%] text-sm ${msg.role === "user" ? "bg-indigo-100 self-end ml-auto" : "bg-gray-200 self-start mr-auto"}`}
              >
            ...
            {isGenerating && (
              <div className="p-3 rounded-lg bg-gray-200 self-start mr-auto">
                <Spinner size="sm" />
              </div>
            )}
          </div>
          ...
          >
            <textarea
              ...
              placeholder={t("roadmapEditor.askAiToChangePlan")}
        ...
        >
          ...
          >
            <h2 className="text-xl font-bold mb-4">
              {t("roadmapEditor.studentAnswers", {
                name:
                  currentGoal?.student?.firstName || t("roadmapEditor.student"),
              })}
            </h2>
            <div className="overflow-y-auto flex-grow pr-4">
              {isLoadingLogs ? (
                  ...
                  {(performanceLogs as PerformanceLog[]).map((log) => (
                    ...
                    >
                      <p className="font-semibold mt-1">
                        {t("roadmapEditor.question")}:{" "}
                        <span className="italic text-gray-700 font-normal">
                          "{log.question}"
                        </span>
                      </p>
                      <p className="font-semibold mt-2">
                        {t("roadmapEditor.studentAnswer")}:{" "}
                ...
                <p className="text-gray-500">
                  {t("roadmapEditor.noStudentAnswers")}
                </p>
              )}
            </div>
          </div>
      ...
      {showStoryModal && (
        ...
        >
          <div
            className="bg-white p-6 rounded-lg shadow-xl max-w-4xl w-full max-h-[80vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-xl font-bold mb-4">
              {t("roadmapEditor.adventureStory")}
            </h2>
            <div className="overflow-y-auto flex-grow pr-4">
              {isLoadingStory ? (
                <Spinner />
                        ...
                        <div className="flex flex-col md:flex-row gap-6 items-start mb-6">
                          {chapter.teacherSnippetImageUrl && (
                            <img
                              ...
                              alt={t("roadmapEditor.chapterIllustration", {
                                number: index + 1,
                              })}
                          ...
                          <div className="flex-1">
                            <p className="font-semibold text-gray-700">
                              {t("roadmapEditor.narrator")}:
                            </p>
                            <p className="mt-2 text-gray-700 italic leading-relaxed whitespace-pre-wrap">
                              {chapter.teacherSnippetText}
                            </p>
                          </div>
                        </div>
