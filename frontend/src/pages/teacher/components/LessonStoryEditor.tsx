...
const LessonStoryEditor = ({
  ...
  const [generateStory, { isLoading: isGeneratingStory }] =
    useGenerateStorySnippetMutation();
  const [regenerateImage, { isLoading: isGeneratingImage }] =
    useRegenerateStoryImageMutation();
  const [approveWithUrl, { isLoading: isApprovingUrl }] =
    useApproveStorySnippetMutation();
  const [approveWithFile, { isLoading: isApprovingFile }] =
    useApproveStorySnippetWithUploadMutation();
  ...
  const handleGenerateImage = async () => {
    if (!imagePrompt) {
      toast.error(t("lessonStoryEditor.promptEmptyError"));
      return;
    }
    setUploadedFile(null);
    setImageUrl(null);
  ...
  const handleApprove = async () => {
    if (!storyText.trim()) {
      toast.error(t("lessonStoryEditor.storyTextEmptyError"));
      return;
    }

    try {
      if (uploadedFile) {
        await approveWithFile({
          lessonId: lesson.id,
          image: uploadedFile,
          text: storyText,
          prompt: imagePrompt,
    ...
    <div className="rounded-xl p-3 max-h-[85vh] flex flex-col">
      <div className="flex-grow overflow-y-auto pr-2 space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t("lessonStoryEditor.storyFragment")}
          </label>
          <textarea
            value={storyText}
          ...
          <button
            ...
            className="mt-2 w-full btn-primary text-sm disabled:opacity-50 flex items-center justify-center gap-2"
          >
            {isGeneratingStory ? (
              <Spinner size="sm" />
            ) : (
            ...
            <span>{t("lessonStoryEditor.generateStoryText")}</span>
          </button>
        </div>

        <div className="border-t pt-4">
          <label className="block text-sm font-medium text-gray-700 mb-1">
            {t("lessonStoryEditor.imagePrompt")}
          </label>
          <textarea
            value={imagePrompt}
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white bg-purple-600 rounded-md hover:bg-purple-700 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {isGeneratingImage || generationId ? (
                <Spinner size="sm" />
              ) : (
                <FiImage size={16} />
              )}{" "}
              <span>
                {isGeneratingImage || generationId
                  ? t("lessonStoryEditor.generating")
                  : t("lessonStoryEditor.createImage")}
              </span>
            </button>
            <button
              onClick={() => fileInputRef.current?.click()}
              disabled={isLoading}
              className="px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-md hover:bg-green-700 disabled:opacity-50 flex items-center justify-center gap-2"
            >
              <FiUpload size={16} />{" "}
              <span>{t("lessonStoryEditor.uploadOwn")}</span>
            </button>
            <input
              type="file"
              ref={fileInputRef}
          ...
          <div className="w-full h-64 bg-gray-100 rounded-lg flex items-center justify-center relative">
            {generationId || isGeneratingImage ? (
              ...
              <span className="text-gray-400">
                {t("lessonStoryEditor.imageWillAppearHere")}
              </span>
            )}
          </div>
        </div>
      ...
      <div className="mt-6 pt-4 border-t flex justify-end">
        <button
          ...
          className="px-6 py-2 text-sm font-bold text-white bg-green-700 rounded-md hover:bg-green-800 disabled:opacity-50 flex items-center gap-2"
        >
          {isApprovingFile || isApprovingUrl ? (
            <Spinner size="sm" />
          ) : (
            <FiCheck size={16} />
          )}{" "}
          <span>{t("lessonStoryEditor.approveAndFinish")}</span>
        </button>
      </div>
    </div>
  );
