...
export const LessonStatusIndicator = ({ lesson }: { lesson: Lesson }) => {
  ...
  const statusMap = {
    ...
    PENDING_APPROVAL: {
      color: "bg-yellow-400",
      title: t("lessonStatusIndicator.pendingApproval"),
    },
    APPROVED: {
      color: "bg-green-500",
      title: t("lessonStatusIndicator.approved"),
    },
    COMPLETED: {
      color: "bg-blue-500",
      title: t("lessonStatusIndicator.completed"),
    },
  };
  const { color, title } =
    ...
    <div className="flex items-center space-x-1.5" title={title}>
      ...
      {lesson.storyChapter?.teacherSnippetStatus === "APPROVED" && (
        <FiBookOpen
          ...
          title={t("lessonStatusIndicator.hasApprovedStory")}
        />
      )}
    </div>
  );
