...
export const ControlWorkContentEditor = ({
  lesson,
  onCloseModal,
}: ControlWorkContentEditorProps) => {
  ...
  const handleGenerateContent = async (e: React.FormEvent) => {
    e.preventDefault();
    const messageToSend =
      userMessage.trim() || t("controlWorkContentEditor.generateInitialTasks");
    const newHistory = [
      ...chatHistory,
    ...
    try {
      ...
      toast.error(
        err.data?.message || t("controlWorkContentEditor.generateError"),
      );
    ...
    <div className="rounded-xl p-3 max-h-[75vh] flex flex-col">
        ...
        {blocks.length > 0 && (
          <div className="space-y-3">
            {blocks.map((block, index) => (
              <div
                key={block.id}
                className="p-4 rounded-md border bg-amber-50 border-amber-200"
              >
                <label className="font-semibold text-gray-700">
                  {t("controlWorkContentEditor.task", { number: index + 1 })}
                </label>
                <textarea
                  value={block.content}
                  onChange={(e) =>
                  ...
                  placeholder={t(
                    "controlWorkContentEditor.taskTextPlaceholder",
                  )}
                />
              </div>
            ))}
      ...
      <div className="mt-4 pt-4 border-t">
        <div
          ...
          className="space-y-2 mb-2 max-h-40 overflow-y-auto p-2 bg-gray-50 rounded"
        >
        ...
        <form onSubmit={handleGenerateContent} className="flex gap-2">
          <textarea
            ...
            placeholder={
              blocks.length > 0
                ? t("controlWorkContentEditor.askAiToRedo")
                : t("controlWorkContentEditor.startGeneration")
            }
      ...
      <div className="mt-6 flex justify-end items-center gap-x-4">
        ...
        >
          ...
          {isSaving
            ? t("controlWorkContentEditor.saving")
            : t("controlWorkContentEditor.save")}
        </button>
      </div>
    </div>
  );
