...
export const LessonContentEditor = ({
  ...
  useEffect(() => {
    const initialBlocks = (lesson.content?.blocks || []).map(
  ...
  const handleBlockChange = (
    index: number,
    field: keyof LessonContentBlock,
    value: string | number,
  ) => {
    const newBlocks = [...blocks];
    (newBlocks[index] as any)[field] =
  ...
  const handleGenerateContent = async (e: React.FormEvent) => {
    e.preventDefault();
    const messageToSend =
      userMessage.trim() || t("lessonContentEditor.generateInitialContent");
    const newHistory = [
      ...chatHistory,
    ...
    try {
      ...
      const blocksWithIds = (responseData.blocks || []).map(
        (b: any, i: number) => ({ ...b, id: `gen-block-${i}-${Date.now()}` }),
      );
      setBlocks(blocksWithIds);
      setChatHistory((prev) => [
        ...prev,
        { role: "assistant", content: responseData.chatResponse },
      ]);
    ...
    <div className="rounded-xl p-3 max-h-[75vh] flex flex-col">
      <div className="flex-grow overflow-y-auto pr-2">
                      ...
                      >
                        <div className="flex justify-between items-center mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`px-2 py-0.5 text-xs font-medium capitalize rounded-full ${block.type === "theory" ? "bg-blue-100 text-blue-800" : block.type === "practice" ? "bg-purple-100 text-purple-800" : "bg-red-100 text-red-800"}`}
                            >
                              {block.type}
                            </span>
                            {block.type !== "youtube" && (
                              <>
                                <input
                                  ...
                                  title={t(
                                    "lessonContentEditor.durationInMinutes",
                                  )}
                                />
                                <span className="text-xs text-gray-500">
                                  {t("lessonContentEditor.minutes")}
                                </span>
                              </>
                            )}
                          </div>
                          <button
                            ...
                            title={t("lessonContentEditor.removeBlock")}
                          >
                            <FiTrash2 size={16} />
                          </button>
                        </div>
                        <textarea
                          ...
                          placeholder={
                            block.type === "youtube"
                              ? t("lessonContentEditor.youtubeLinkPlaceholder")
                              : t("lessonContentEditor.textPlaceholder")
                          }
                        />
                      </div>
                    )}
        ...
        <div className="mt-4 flex gap-2">
          ...
          >
            + {t("lessonContentEditor.theory")}
          </button>
          <button
            onClick={() => addBlock("practice")}
            className="text-sm px-3 py-1 rounded bg-purple-50 hover:bg-purple-100 text-purple-700"
          >
            + {t("lessonContentEditor.practice")}
          </button>
          <button
            onClick={() => addBlock("youtube")}
            className="text-sm px-3 py-1 rounded bg-red-50 hover:bg-red-100 text-red-700"
          >
            + YouTube
          </button>
        </div>
      ...
      <div className="mt-4 pt-4 border-t">
        <div className="space-y-2 mb-2 max-h-40 overflow-y-auto">
          {chatHistory.map((msg, idx) => (
            <div
              key={idx}
              className={`text-sm p-2 rounded-lg ${msg.role === "user" ? "bg-gray-200 ml-auto" : "bg-blue-100"}`}
              style={{ maxWidth: "85%" }}
            >
              {msg.content}
            </div>
          ...
          {isGenerating && (
            <div
              className="text-sm p-2 rounded-lg bg-blue-100"
              style={{ maxWidth: "85%" }}
            >
              <Spinner size="sm" />
            </div>
        ...
        >
          <textarea
            ...
            placeholder={
              blocks.length > 0
                ? t("lessonContentEditor.askAiToRedoContent")
                : t("lessonContentEditor.askAiToMakeContent")
            }
      ...
      <div className="mt-6 flex justify-end gap-x-4">
        ...
        >
          {isSaving
            ? t("lessonContentEditor.saving")
            : t("lessonContentEditor.saveContent")}
        </button>
      </div>
    </div>
  );
