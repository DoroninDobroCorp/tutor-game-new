...
import { FiPlus, FiTrash2, FiMove, FiSend } from 'react-icons/fi';
...
export const LessonContentEditor = ({ lesson, onCloseModal }: LessonContentEditorProps) => {
    ...
    const handleGenerateContent = async (e: React.FormEvent) => {
        e.preventDefault();
        const messageToSend = userMessage.trim() || 'Generate initial content';
                                            ...
                                            <div ref={provided.innerRef} {...provided.draggableProps} className="p-4 rounded-md border bg-gray-50">
                                                <div className="flex justify-between items-center mb-2">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {block.type !== 'youtube' && (<>
                                                          <input type="number" value={block.duration} onChange={(e) => handleBlockChange(index, 'duration', e.target.value)} className="w-16 p-1 text-xs border rounded-md" title="Длительность в минутах" />
                                                ...
                                                <textarea value={block.content} onChange={(e) => handleBlockChange(index, 'content', e.target.value)} rows={block.type === 'youtube' ? 1 : 4} className="w-full p-2 border border-gray-300 rounded-md text-sm mt-2" placeholder={block.type === 'youtube' ? 'Вставьте полную ссылку на YouTube видео...' : 'Введите текст...'} />
                                            </div>
                                        )}
                                    </Draggable>
                                ))}
