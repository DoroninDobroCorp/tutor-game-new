  ...
  endpoints: (builder) => ({
    ...
    getGoalStoryHistory: builder.query<StoryChapterHistory[], string>({
      ...
      providesTags: (result, _error, goalId) =>
        result
          ? [{ type: "Goal" as const, id: `STORY_HISTORY_${goalId}` }]
          : [],
    }),

    generateRoadmapProposal: builder.mutation<
      RoadmapProposal,
      {
        goalId: string;
        chatHistory: any[];
    ...
    generateCharacterForGoal: builder.mutation<
      LearningGoal,
      { goalId: string; prompt: string }
    >({
      query: ({ goalId, prompt }) => ({
    ...
    uploadCharacterImage: builder.mutation<
      LearningGoal,
      { goalId: string; image: File; prompt?: string }
    >({
    ...
    updateCharacterPrompt: builder.mutation<
      LearningGoal,
      { goalId: string; prompt: string }
    >({
      query: ({ goalId, prompt }) => ({
