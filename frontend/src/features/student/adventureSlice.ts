...
const adventureSlice = createSlice({
  ...
  reducers: {
    ...
    updatePracticeAnswer: (
      state,
      action: PayloadAction<{ blockIndex: number; answer: string }>,
    ) => {
...
export const selectPracticeAnswers = (state: RootState) =>
  state.adventure.practiceAnswers;
export const selectStoryResponse = (state: RootState) =>
  state.adventure.storyResponse;
