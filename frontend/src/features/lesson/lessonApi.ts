...
export const lessonApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    generateLessonContent: builder.mutation<
      { data: { chatResponse: string; blocks: any[] } },
      { lessonId: string; chatHistory: any[] }
    >({
    ...
    generateControlWorkContent: builder.mutation<
      { data: { chatResponse: string; blocks: any[] } },
      { lessonId: string; chatHistory: any[] }
    >({
      query: ({ lessonId, chatHistory }) => ({
    ...
    generateStorySnippet: builder.mutation<
      ...
      { lessonId: string; [key: string]: any }
    >({
      query: ({ lessonId, ...data }) => ({
    ...
    approveStorySnippet: builder.mutation<
      StoryChapter,
      { lessonId: string; [key: string]: any }
    >({
      query: ({ lessonId, ...data }) => ({
    ...
    approveStorySnippetWithUpload: builder.mutation<
      StoryChapter,
      { lessonId: string; image: File; text: string; prompt: string }
    >({
    ...
    regenerateStoryImage: builder.mutation<
      { data: { generationId: string; prompt: string } },
      { lessonId: string; prompt: string; useCharacterReference: boolean }
    >({
